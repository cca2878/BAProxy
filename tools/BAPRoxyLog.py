...
def BAPoxyLog(*args, **kwargs) -> Callable:
    def BAProxy_insert_logger(cls):
        class BAProxyLogger:
            ...
            def __call__(self, msg: str, level: str = 'info') -> None:
                ...
                try:
                    ...
                    logging.getLogger('BAProxyLogger').error(self.__color_table['error'].format(
                        f"No such log level(log level: {level}) for log: {msg}"))
                    return
                log_func(log_color.format(msg), stacklevel=2)

        cls.logger = BAProxyLogger()
        return cls

    if args and inspect.isclass(args[0]):
        return BAProxy_insert_logger(args[0])
    return BAProxy_insert_logger
