...
from BAProxyMode import BAProxyMode
from tools.FlowUtils import adaptive_decode, adaptive_encode, get_active_part, is_pcr_api
from tools.BAPRoxyLog import BAPoxyLog


class FuncNode:
    def __init__(self, func: Callable, plugin_name: str, mode_list: List[BAProxyMode], path: str, priority: int) -> None:
        self.func = func
...
class FlowChain:
    ...
    @abstractmethod
    async def run_flow(self, flow: http.HTTPFlow, mode: BAProxyMode):
        raise RuntimeError()
...
class HookChain(FlowChain):
    async def run_flow(self, flow: http.HTTPFlow, mode: BAProxyMode):
        ...
        if mode == BAProxyMode.MODIFIER and context.payload != origin_data:
